use super::*;
use crate::test_helpers::fixtures::{empty_jwks, multi_key_jwks, single_rsa_jwks};
use crate::test_helpers::http::jwks_server;
...
fn stores_empty_jwks_set() {
    let jwks: JwkSet = serde_json::from_str(empty_jwks()).unwrap();
    let service = ClerkAuthService {
        jwks,
        frontend_api_url: "https://musical-pup-67.clerk.accounts.dev".to_string(),
    };
...
fn finds_jwk_by_kid_when_present() {
    let jwks: JwkSet = serde_json::from_str(multi_key_jwks()).unwrap();
    let service = ClerkAuthService {
        jwks,
        frontend_api_url: "https://musical-pup-67.clerk.accounts.dev".to_string(),
    };
...
async fn new_successfully_creates_service_with_valid_jwks() {
    let server = jwks_server(single_rsa_jwks()).await;
...
async fn new_builds_jwks_url_from_frontend_api_url() {
    let server = jwks_server(multi_key_jwks()).await;
